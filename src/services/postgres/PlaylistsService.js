...
class PlaylistsService {
  ...
  async addSongToPlaylist(playlistId, songId) {
    try{
    // this.verifySongExist(songId); //Verify apakah song yg ditambahkan ke dlm playlist tersedia di database songs atau tidak
  ...
  async getPlaylistbyId(playlistId, credentialId) {
    const query = {
      ...
      ON playlists.id = playlistsongs.playlist_id WHERE playlistsongs.playlist_id = $1
      INNER JOIN users
      ON playlists.owner = users.id WHERE users.id = $2`,
